...
def procesar_eventos():
    ...
    while True:
        ...
        if evento == CONEXION_FALLIDA:
            ...
            MQTT_TOPIC = [("/DATA/OUT/HUMEDAD_SUELO/STAGE0/VALUE",0)]
            ...
            for n in data_in:
                media += n
            media=media/len(data_in)
            data_in.append(round(media, 2))
            data_out=json.dumps(data_in)
            # fin procesamiento de este dispositivo
            conexion_cliente.publish("/DATA/OUT/HUMEDAD_SUELO/STAGE1/VALUE", data_out)


start_new_thread(procesar_eventos,())

while(not conectado):
    print("DispStage1Sensor3")
    print("Conectando...")
    try:
