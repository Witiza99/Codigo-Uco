...
def procesar_eventos():
    ...
    while True:
        ...
        if evento == CONEXION_FALLIDA:
            ...
            #MQTT_TOPIC = [("/DATA/OUT/TEMPERATURA/STAGE0/VALUE",0),("/DATA/OUT/HUMEDAD_AIRE/STAGE0/VALUE",0),("/DATA/OUT/HUMEDAD_SUELO/STAGE0/VALUE",0)]
            MQTT_TOPIC = [("/DATA/OUT/HUMEDAD_SUELO/STAGE2/VALUE",0)]
            ...
            desv_tipica = math.sqrt(desv_tipica)

            data_in.append(round(desv_tipica, 2))
            data_out=json.dumps(data_in)
            # fin procesamiento de este dispositivo
            conexion_cliente.publish("/DATA/OUT/HUMEDAD_SUELO/STAGE3/VALUE", data_out)


start_new_thread(procesar_eventos,())

while(not conectado):
    print("DispStage3Sensor3")
    print("Conectando...")
    try:
