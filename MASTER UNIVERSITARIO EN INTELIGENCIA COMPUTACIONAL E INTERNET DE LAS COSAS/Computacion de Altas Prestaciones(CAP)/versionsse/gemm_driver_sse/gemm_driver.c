...
/*
#define Tcol(a1, a2) Tc[a2 * ldTc + a1]
#define Trow(a1, a2) Tr[a1 * ldTr + a2]

void rowToCol(DT *Tc, DT *Tr, size_t rows, size_t cols) {
...
void colToRow(DT *Tc, DT *Tr, size_t rows, size_t cols) {
...
int main(int argc, char **argv) {
  ...
  for (l = 0; l < nlayers; l++) { 
    ...
    if (errorREF <= errorthd) 
      ...
      printf("  %s%10.2e%s      | \n", COLOR_RED, errorREF, COLOR_RESET );
      exit(-1);
    }
