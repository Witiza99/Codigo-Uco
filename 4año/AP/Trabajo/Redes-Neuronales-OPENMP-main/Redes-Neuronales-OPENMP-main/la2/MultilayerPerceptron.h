...
private:
	...
	void feedInputs(double* input, int n);
	...
	void backpropagateError(double* target, int n);

	// Accumulate the changes produced by one pattern and save them in deltaW
	void accumulateChange(int n);

	// Update the network weights, from the first layer to the last one
	void weightAdjustment();

	void weightAdjustmentOnline(int n);
	...
	void performEpoch(double* input, double* target, int n);
...
public:
	...
	int initialize(int nl, int npl[], int nTh);
	...
	void train(Dataset* trainDataset);

	void printConfusionMatrix(Dataset* dataset);
	...
	void runBackPropagation(Dataset * trainDataset, Dataset * testDataset, int maxiter, double *errorTrain, double *errorTest, double *ccrTrain, double *ccrTest, int *count, int i);
