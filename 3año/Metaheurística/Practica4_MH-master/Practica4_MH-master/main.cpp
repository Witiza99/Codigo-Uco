 ...
 */
void runAGAExperiment(vector<double> &currentResults, vector<double> &bestSoFarResults, vector<double> &bestPerIterations, vector<double> &popMean, vector<double> &offMean, MQKPInstance &instance) {
	//Inicialización
 ...
 * @param[in] instance Instancia del problema de la mochila cuadrática múltiple
 */
void runAnACOExperiment(vector<double> &currentResults, vector<double> &bestSoFarResults, vector<double> &bestPerIterations, vector<double> &antsMean, MQKPInstance &instance) {
	//Inicialización
...
void runExperiments(vector<vector<vector<double>*>*> &results, char **mainArgs,
	...
	for (unsigned int iInstance = 1; iInstance < numMainArgs; iInstance += 2) {
		...
		runAGAExperiment(*theseResults, *bestResults, *bestPerIterations, *popMeanResults, *offMeanResults, instance);
