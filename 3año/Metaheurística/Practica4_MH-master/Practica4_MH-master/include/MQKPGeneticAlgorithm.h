...
protected:
	...
	unsigned indexBest(vector<Solution*> &set) {
		...
		for (unsigned i = 1; i < set.size(); i++) {
			if (MQKPEvaluator::compare(set[indexBest]->getFitness(), set[i]->getFitness()) > 0) {
				indexBest = i;
			}
		}
		return indexBest;
	...
	unsigned indexWorst(vector<Solution*> &set) {
		...
		for (unsigned i = 1; i < set.size(); i++) {
			if (MQKPEvaluator::compare(set[indexWorst]->getFitness(), set[i]->getFitness()) < 0) {
				indexWorst = i;
			}
		}
		return indexWorst;
	...
	void initPopulation(unsigned popSize) {
		...
		for (unsigned i = 0; i < popSize; i++) {
			...
			if (MQKPEvaluator::compare(_bestSolution->getFitness(), sol->getFitness()) < 0) {
