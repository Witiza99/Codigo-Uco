...
protected:
	...
	Solution* selectOne (vector<Solution*> &set){
		...
		MQKPSolution * best = (MQKPSolution*) set.at(randSample);
		for (unsigned i = 0; i < _k; i++) {
			randSample = rand()%set.size();
			MQKPSolution * next = (MQKPSolution*) set.at(randSample);
			if (MQKPEvaluator::compare(best->getFitness(), next->getFitness()) > 0) {
				best = next;
			}
		}
		return best;
