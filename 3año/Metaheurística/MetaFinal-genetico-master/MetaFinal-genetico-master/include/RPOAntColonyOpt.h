 ...
 * RPOAntColonyOpt.h
 *
 * Fichero que define la clase RPOAntColonyOpt. Forma parte del código esqueleto para el problema de las múltiples mochilas cuadráticas, ofrecido para las prácticas de la asignatura Metaheurísticas del Grado de Ingeniería Informática de la Universidad de Córdoba
 *
 */
...
/**
 * Clase que implementa un sistema de colonias de  hormigas para el RPO.
 */

class RPOAntColonyOpt: public RPOMetaheuristic {

	/**
	 * Clase que define implementa internamente a una hormiga para el RPO
	 */
	class RPOAnt {
	protected:
		 ...
		 */
		void createAlternatives(vector<RPOObjectAssignmentOperation*> &alternatives, vector<double> &significances) {
			...
			for (auto indexObj : _objectsLeft) {
				...
				for (unsigned j = 1; j <= numKnapsacks && numTries < _candidateListSize; j++) {
					 ...
					 */
					RPOObjectAssignmentOperation *al = new RPOObjectAssignmentOperation();
					al->setValues(indexObj, j, deltaFitness);
					double density = deltaFitness/instance->getWeight(indexObj);
					double pheromone = (double) phMatrix[indexObj]->at(j);
					double significance = pow(density, beta) * pow(pheromone, alpha);
	...
	public:
		...
		void resetSolution() {
			 ...
			 */
			 for (int i = 0; i < _colony->_instance->getNumObjs(); i++) {
		...
		void chooseOperation(RPOObjectAssignmentOperation &operation) {
			...
			if (randSample < _colony->_q0) {
				...
				double randSample = (((double) rand()) / RAND_MAX) * v_sumSignificances;
				unsigned opSelected = 0;
				for (unsigned i = 0; i < alternatives.size(); i++) {
					randSample -= significances.at(i);
					if (randSample < 0) {
						opSelected = i;
						i = alternatives.size();
					}
...
protected:
	...
	void localUpdate(RPOObjectAssignmentOperation &op) {
		_phMatrix[op.getObj()]->at(op.getKnapsack()) = _phMatrix[op.getObj()]->at(op.getKnapsack()) + _initTau;
	}
	...
	void iterate() {
		...
		for (unsigned i = 0; i < numObjs; i++) {
			_phMatrix[i]->at(_bestSolution->whereIsObject(i)) = _initTau;
		}
	}
...
public:
	...
	void initialise(unsigned numAnts, double q0, double alpha, double beta,
		...
		for (unsigned i = 0; i < numAnts; i++) {
			RPOAnt *ant = new RPOAnt(candidateListSize , this);
			_ants.push_back(ant);
		}
