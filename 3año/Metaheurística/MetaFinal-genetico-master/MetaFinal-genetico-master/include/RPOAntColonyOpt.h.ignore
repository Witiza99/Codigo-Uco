...
class MQKPAntColonyOpt: public MQKPMetaheuristic {
	...
	protected:
		 ...
		 */
		void createAlternatives(vector<MQKPObjectAssignmentOperation*> &alternatives, vector<double> &significances) {
			...
			for (auto indexObj : _objectsLeft) {
				...
				for (unsigned j = 1; j <= numKnapsacks && numTries < _candidateListSize; j++) {
					 ...
					 */
					MQKPObjectAssignmentOperation *al = new MQKPObjectAssignmentOperation();
