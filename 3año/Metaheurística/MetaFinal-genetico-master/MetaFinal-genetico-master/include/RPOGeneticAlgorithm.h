/*
 * RPOGeneticAlgorithm.h
 *
 * Fichero que define la clase RPOGeneticAlgorithm. Forma parte del código esqueleto para el problema de las múltiples mochilas cuadráticas, ofrecido para las prácticas de la asignatura Metaheurísticas del Grado de Ingeniería Informática de la Universidad de Córdoba
 *
 * @author Carlos García cgarcia@uco.es
 */

#ifndef INCLUDE_RPOGENETICALGORITHM_H_
#define INCLUDE_RPOGENETICALGORITHM_H_

#include <RPOMetaheuristic.h>
#include <RPOSolution.h>
#include <SelectionOperator.h>
#include <RPOCrossoverOperator.h>
#include <RPOMutationOperator.h>
...
/**
 * Clase que implementa un Algoritmo Genético Generacional con Elitismo para el RPO
 */
class RPOGeneticAlgorithm: public RPOMetaheuristic {
protected:
	/**
	...
	RPOCrossoverOperator *_crossoverOp = NULL;
	RPOMutationOperator *_mutOp = NULL;
	RPOInstance *_instance = NULL;
	...
	unsigned indexBest(vector<Solution*> &set) {
		...
		for (unsigned i = 1; i < set.size(); i++) {
			if (RPOEvaluator::compare(set[indexBest]->getFitness(), set[i]->getFitness()) > 0) {
				indexBest = i;
			}
		}
		return indexBest;
	...
	unsigned indexWorst(vector<Solution*> &set) {
		...
		for (unsigned i = 1; i < set.size(); i++) {
			if (RPOEvaluator::compare(set[indexWorst]->getFitness(), set[i]->getFitness()) < 0) {
				indexWorst = i;
			}
		}
		return indexWorst;
	...
	void selectNewPopulation(vector<Solution*> &offspring) {
		...
		//Si la mejor solución de _population tiene un fitness mayor que la de offspring cambiamos la peor de offspring
		//por la mejor de population
		if (RPOEvaluator::compare(_population[indexBestPop]->getFitness(), offspring[indexBestOff]->getFitness()) > 0) {
			unsigned int indexWorstOff = indexWorst(offspring);
			offspring[indexWorstOff] = _population[indexBestPop];
		}
	...
	void evaluate(vector<Solution*> &set) {
		...
		for (Solution *sol : set) {
			...
			if (!(s->hasValidFitness())) {
				...
				//Evaluar
				double fitness = RPOEvaluator::computeFitness(*_instance ,*s);
				_results.push_back(fitness);
				s->setFitness(fitness);

				//Actualizar la mejor solución
				if (RPOEvaluator::compare(fitness, _bestSolution->getFitness()) > 0){
	...
	void initPopulation(unsigned popSize) {
		...
		for (unsigned i = 0; i < popSize; i++) {
			...
			RPOSolGenerator::genRandomSol(*_instance, *sol);
			fitness = RPOEvaluator::computeFitness(*_instance, *sol);
			sol->setFitness(fitness);
			if (RPOEvaluator::compare(_bestSolution->getFitness(), sol->getFitness()) < 0) {
...
public:
	 ...
	 */
	void initialise(unsigned popSize, RPOInstance &instance) {
		_instance = &instance;
		...
		double fitness = RPOEvaluator::computeFitness(*_instance, *_bestSolution);
		...
		if (_crossoverOp == NULL) {
			_crossoverOp = new RPOCrossoverOperator(0.8, *_instance);
		}

		if (_mutOp == NULL) {
			_mutOp = new RPOMutationOperator((0.25 / _instance->getNDims()),
					*_instance);
		}
	 ...
	 */
	void setCrossoverOp(RPOCrossoverOperator* crossoverOp) {
	 ...
	 */
	void setMutOp(RPOMutationOperator* mutOp) {
...
#endif /* INCLUDE_RPOGENETICALGORITHM_H_ */
