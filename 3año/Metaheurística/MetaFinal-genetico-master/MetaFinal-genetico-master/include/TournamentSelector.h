 ...
 *
 * Fichero que define la clase RPOTournamentSelector. Forma parte del código esqueleto para el problema de las múltiples mochilas cuadráticas, ofrecido para las prácticas de la asignatura Metaheurísticas del Grado de Ingeniería Informática de la Universidad de Córdoba
 *
...
protected:
	...
	Solution* selectOne (vector<Solution*> &set){
		 ...
		 */
		double randSample = rand()%set.size();
		RPOSolution * best = (RPOSolution*) set.at(randSample);
		for (unsigned i = 0; i < _k; i++) {
			randSample = rand()%set.size();
			RPOSolution * next = (RPOSolution*) set.at(randSample);
			if (RPOEvaluator::compare(best->getFitness(), next->getFitness()) > 0) {
				best = next;
			}
		}
		return best;
...
public:
	...
	virtual void select(vector<Solution*> &orig, vector<Solution*> &result){
		...
		//TODO utilizando el método propio selectOne, seleccionar tantas parejas
