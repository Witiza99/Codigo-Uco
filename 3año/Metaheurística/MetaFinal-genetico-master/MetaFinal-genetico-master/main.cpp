 ...
 */
void runAGAExperiment(vector<double> &currentResults, vector<double> &bestSoFarResults, vector<double> &bestPerIterations, vector<double> &popMean, vector<double> &offMean, RPOInstance &instance) {
	//Inicialización
...
int main(int argc, char **argv) {
	...
	if (argc < 3 || (argc % 2) != 1) {
		cout << argv[0] << " (<extdatadir> <numInstance>)+"
				<< endl;
		return 0;
	}
...
//	vector<unsigned> printColumns = { 2, 3, 4, 7, 8};
	...
	for (unsigned int iInstance = 0; iInstance < numInstances; iInstance++) {
		...
		for (auto indexColumnPerInstance : printColumns) {
			cout <<  titles[indexColumnPerInstance] << "\t";
