 ...
 *
 * It is a part of the skeleton code provided for the RPO, as a part of the module Metaheuristics of the
...
void runARandomSearchExperiment(vector<double> &results, RPOInstance &instance){
	...
	while (t.elapsed_time(Timer::VIRTUAL) <= MAX_SECONS_PER_RUN && numSolutions < MAX_SOLUTIONS_PER_RUN){
		...
		double currentFitness = RPOEvaluator::computeFitness(instance, currentSolution);
