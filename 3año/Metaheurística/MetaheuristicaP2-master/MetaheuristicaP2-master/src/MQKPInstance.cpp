...
double MQKPInstance::getDeltaSumProfits(MQKPSolution& solution, int indexObject,
	 ...
	 */
	 if (solution.whereIsObject(indexObject)!=0) {
		for (int i = 0; i < this->getNumObjs(); i++) {
			if (solution.whereIsObject(indexObject)==solution.whereIsObject(i)) {
				deltaSumProfits-=getProfit(i,indexObject);
			}
		}
	 }
	 ...
	 */
	 if (indexKnapsack!=0) {
		deltaSumProfits+=getProfit(indexObject);
		for (int i = 0; i < this->getNumObjs(); i++) {
			if (solution.whereIsObject(i)==indexKnapsack and i!=indexObject) {
				deltaSumProfits+=getProfit(i,indexObject);
			}
		}
	 }
...
double MQKPInstance::getDeltaMaxCapacityViolation(MQKPSolution& solution,
	 ...
	 */
	 int auxKnapsack = solution.whereIsObject(indexObject);
	 double auxMaxCapacityViolation = this->getMaxCapacityViolation(solution);
	 solution.putObjectIn(indexObject, indexKnapsack);
	 double auxMaxCapacityViolationUpdated = this->getMaxCapacityViolation(solution);
	 solution.putObjectIn(indexObject, auxKnapsack);
	 return auxMaxCapacityViolationUpdated - auxMaxCapacityViolation;
}
