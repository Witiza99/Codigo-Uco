...
//New in final work repeat, for, write, read, writestring, readstring
%type <st> stmt asgn write writestring read readstring if while repeat for place clear
...
// FINAL WORK: KEYWORDS
%token READ READSTRING WRITE WRITESTRING IF THEN ELSE ENDIF WHILE DO ENDWHILE REPEAT UNTIL FOR FROM TO STEP ENDFOR CLEAR PLACE
...
if:	/* Simple conditional statement */
  ...
  | IF controlSymbol LPAREN CONSTANT RPAREN THEN stmtlist  ELSE stmtlist ENDIF
	 {
     lp::ConstantNode *aux = new lp::ConstantNode($4);
		// Create a new if statement node
