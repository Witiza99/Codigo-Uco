...
*/
var paginas = ["CategoriesView","misProyectosView","newProjectView","Detalle_Proyecto","donateView","Contactar","faq"];
...
window.onload = function(){	
	...
	if(nuevo === "cvResumeView"){
		...
		document.getElementsByName("cancelar")[0].addEventListener("click",function(evt){
			document.formcurriculum.name.focus();
...
function buttonSession(pag,vector){
...
function verificarIdiomas(){
    ...
    for (var i = 0; i < document.formcurriculum.languages.length; i++) {
        if(document.formcurriculum.languages[i].checked){
            errIdiomas = true;
            i=document.formcurriculum.languages.length;
        }
    }
    if(!errIdiomas){
...
function verificarProgramacion(){
    ...
    for (var i = 0; i < document.formcurriculum.programming.length; i++) {
        if(document.formcurriculum.programming[i].checked){
            errProgramacion = true;
            i=document.formcurriculum.programming.length;
        }
    }
    if(!errProgramacion){
